...
namespace Protecta.Application.Service.Controllers.Cobranzas
{
    ...
    public class CobranzaController : Controller
    {
        ...
        public async Task<IActionResult> InsertarRecSelFormaPagoTemp([FromBody] ObjParameter objs)
        {
            _logger.LogInfo("Metodo Inserta en temporal la lista seleccionada");

            try
            {
                var result = await _cobranzaService.InsertarRecSelFormaPagoTemp(objs.ConsultaInsertListaFPTemp);
                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogInfo("Metodo Inserta en tabla los marcados " + ex.Message);
                return Json(new { status = "error", message = ex.Message });
            }
        ...
        public async Task<IActionResult> GetListaSelFPTemp([FromBody] ConsultaListaFPSelTempDto consultaListaFPSelTempDto)
        {
            _logger.LogInfo("Metodo obtiene listado temporal");

            try
            {
                var result = await _cobranzaService.GetListaSelFPTemp(consultaListaFPSelTempDto);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost("[action]")]
        public async Task<IActionResult> ProcessGenerarPlanillaNC([FromBody] PlanillaFPDto planillaFPDto)
        {
            _logger.LogInfo("Metodo que genera la planilla según la forma de Pago seleccionada");
            try
            {
                var result = await _cobranzaService.ProcessGenerarPlanillaFP(planillaFPDto);
                return Ok(result);
            }
            catch (Exception ex)
            {
                _logger.LogInfo("Método genera la planilla según la forma de Pago seleccionada " + ex.Message);
                return Json(new { status = "error", message = ex.Message });
            }
